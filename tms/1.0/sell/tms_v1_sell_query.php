...
?>
	<tr id="<?php echo $lineNum?>" bgcolor="#CCCCCC">
		...
		<td style="display:none"><?=$remainMoney?></td>
		<td style="display:none"><?=$fromstation?></td>
		<td style="display:none"><?=$reachstation?></td>
	...
	-->
			[<a href="tms_v1_sell_sellview.php?i=<?=$rows['tml_NoOfRunsID']?>&d=<?=$selldate?>&f=<?=$fromstation?>&r=<?=$reachstation?>&t=<?=$rows['pd_BeginStationTime']?>&l=<?=$rows['tml_LeaveSeats']?>&h=<?=$rows['tml_LeaveHalfSeats']?>&p=<?=$rows['pd_FullPrice']?>&m=<?=$rows['tml_BusModel']?>&a=<?=$rows['tml_Allticket']?>&rm=<?=$remainMoney?>">开始售票</a>]
		</td>
	</tr>
<?php 
	}
