	...
	function doubleclick(target,str){  //双击事件
		...
		for(var i=1;i<sTable.rows.length;i++){
			if (sTable.rows[i]==target){
				...
				opener.document.getElementById("resetreason").value=document.getElementById("RegionCode1").value;
				window.close();
			}
		}  
	}
	function delresult(){ //删除记录
		if (!document.getElementById("RegionCode1").value){
			...
			if(!confirm("确定要删除此记录吗？")){
				...
				jQuery.get(
						'tms_v1_sell_delresetresult.php',
    ...
    <body>
    	<table width="100%" align="center" class="main_tableboder" border="1" cellpadding="3" cellspacing="1">
			<tr >
				...
				<span class="graytext" style="margin-left:8px;">重置票号原因  </span>
				</td>
			</tr>
		</table>
		...
		<table width="100%" align="center" class="main_tableboder" border="1" cellpadding="3" cellspacing="1">
			<tr>
				...
				<td align="center" nowrap="nowrap" bgcolor="#FFFFFF">
					...
					<input type="submit" name="submit1" value="增加" onClick="AddInfo('tms_v1_sell_addresetreason.php')">&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<div id="tableContainer" class="tableContainer"> 
      	  ...
      	  // if(!isset($_REQUEST['submit2'])){
      	   $name=$_REQUEST['Name'];
      	   $str="SELECT * FROM tms_ticket_ResetResult WHERE rr_mncode like '%$name%'";
