...
$update="UPDATE tms_bd_TicketMode SET tml_SeatStatus='{$rowmode[0]}',tml_LeaveSeats='{$rowmode[1]}' WHERE  tml_NoOfRunsID='{$rowweb[1]}' AND tml_NoOfRunsdate='{$rowweb[2]}'";
...
if ($querymode && $querydel && $queryupdate) {
	...
	echo "<script>alert('取消成功！ 请返回。');location.assign('tms_v1_websell_websell.php');</script>";
}else{
	$class_mysql_default->my_query("ROLLBACK");
	echo "<script>alert('取消失败！');location.assign('tms_v1_websell_websearchreserve.php');</script>";
} 
