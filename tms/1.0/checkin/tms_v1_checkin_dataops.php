...
switch ($op)
{
	...
	case "CONFIRMCHECK":
		...
		if(mysql_num_rows($queryReturn) == 1){
			$rowReturn=mysql_fetch_array($queryReturn);
		...
		while($rows = mysql_fetch_array($result)) {
			...
			if(mysql_num_rows($result1) == 1) {
				$rowsell=mysql_fetch_array($result1);
				...
				if(mysql_num_rows($resultsellticket) == 1) {
					$rowsellticket=mysql_fetch_array($resultsellticket);
	...
	/*	$queryString = "INSERT `tms_chk_CheckTicketTemp` (`ctt_TicketID`, `ctt_NoOfRunsID`, `ctt_LineID`, `ctt_NoOfRunsdate`, 
		...
		if(!$result) {
			...
			$retData = array('retVal' => 'FAIL', 'retString' => '检票数据添加失败！'.mysql_error(), 'sql' => $queryString);
			echo json_encode($retData);
			exit();
		}
	...
	case "CONFIRMCHECKALL":
		...
		if(!$result) {
			$class_mysql_default->my_query("ROLLBACK");
			$retData = array('retVal' => 'FAIL', 'retString' => '检票数据添加失败1！'.mysql_error(), 'sql' => $queryString);
			echo json_encode($retData);
			exit();
		}
		//检查报班车辆是否与票版中车辆一致
	...
	/*	$queryString = "SELECT `ctt_TicketID`, `ctt_NoOfRunsID`, `ctt_LineID`, `ctt_NoOfRunsdate`, `ctt_FromStationID`, `ctt_FromStation`, 
		...
		while ($rows2 = mysql_fetch_array($result_ctt)) {
			if($SeatIDstr==''){
...
/*	case "CONFIRMPRINT":
	...
	case "CONFIRMPRINT":
		...
		if($opp!='reprint' || $error=='1'){
			...
			$rowticketmodel= mysql_fetch_array($queryticketmodel);
			$class_mysql_default->my_query("BEGIN");
			if($oldBalanceNo!=''){
				if($error==1){
				  	...
				  	if(!$result) {
						...
						if(!$resultLuggageCons ){
							...
							if(!$result){
								...
								$printData = "";
								while($rows4 = mysql_fetch_array($results)){
...
//								else 
	...
	case "changebus";
		...
		if(mysql_num_rows($resultSafetyCheck)==0){
			$remark=$remark.'没安检；';
		}else{
	...
	//	$rowmode= mysql_fetch_array($querymode);		
		...
		while($rows2 = mysql_fetch_array($resultck)){
		...
		if(!$queryrt){
			$class_mysql_default->my_query("ROLLBACK");
			$retData = array('retVal' => 'FAIL', 'retString' => '更新调度信息失败！'.mysql_error(), 'sql' => $queryString);
			echo json_encode($retData);
			exit();
		}
		...
		if(!$queryct){
			$class_mysql_default->my_query("ROLLBACK");
			$retData = array('retVal' => 'FAIL', 'retString' => '更新车辆检票信息失败！'.mysql_error(), 'sql' => $queryString);
			echo json_encode($retData);
			exit();
		}   
		$class_mysql_default->my_query("COMMIT");
