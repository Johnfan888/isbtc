...
$rowsticket = @mysql_fetch_array($resultselet);
	if (empty($rowsticket[0])) {
		echo "<script>alert('没有可用的结算单！'); location.assign('tms_v1_checkin_querybalance.php');</script>";
	}
?>
		...
		function testing(){
			...
			if(document.getElementById("resetticketno").value!=''){
				...
				if(parseInt(document.getElementById("resetticketno").value)>parseInt(document.getElementById("endticketno").value)+1){
					alert("重置结算单号大于结束票号"+document.getElementById("endticketno").value);
					document.getElementById("resetticketno").value='';
					document.getElementById("resetticketno").focus();
					return false;
				}
				if(parseInt(document.getElementById("resetticketno").value)<=parseInt(document.getElementById("ticketno").value)){
					alert("重置结算单号不能小于等于当前票号"+document.getElementById("ticketno").value);
					document.getElementById("resetticketno").value='';
					document.getElementById("resetticketno").focus();
					return false;
				}
				...
				if(!confirm("确认将当前结算单号:"+document.getElementById("ticketno").value+" 重置到 "+document.getElementById("resetticketno").value+"？")){
					return false;
				}
			}
...
<table width="100%" align="center" class="main_tableboder" border="1" cellpadding="3" cellspacing="1">
  <tr>
    ...
    <span class="graytext" style="margin-left:8px;">重 置 结 算 单 界 面</span></td>
  </tr>
</table>
<form action="" method="post" name="form1">
<table width="40%" align="center" class="main_tableboder" border="1" cellpadding="3" cellspacing="1">
  <tr>
    <td nowrap="nowrap" bgcolor="#FFFFFF"><span class="form_title"><img src="../ui/images/sj.gif" width="6" height="7" />当前结算单号：</span></td>
  ...
  <tr>
    <td nowrap="nowrap" bgcolor="#FFFFFF"><span class="form_title"><img src="../ui/images/sj.gif" width="6" height="7" />重置结算单号：</span></td>
  ...
  <tr>
    <td align="center" colspan="2" bgcolor="#FFFFFF">
	    ...
	    <input type="button" name="back" id="back" value="返回"  onclick="location.assign('tms_v1_checkin_querybalance.php')"/>
    </td>
  </tr>
</table>
</form>
	...
	if(isset($_POST['resetticketno'])){
		...
		if($resetticketno!=''){
			$updateticket="UPDATE tms_bd_TicketProvide SET tp_CurrentTicket='{$resetticketno}', tp_InceptTicketNum=(tp_EndTicket+0)-('{$resetticketno}'+0)+1
				WHERE tp_CurrentTicket='{$ticketno}' AND tp_Type = '结算单' AND tp_InceptUserID = '$userID'";
			...
			$insert="INSERT INTO `tms_sell_ResetTicket` (`rt_ID`,`rt_ResetUserID`,`rt_ResetUser`,`rt_UserSation`,`rt_ResetDate`,`rt_BeginTicket`,`rt_CurrentTicket`,`rt_EndTicket`,`rt_InceptTicketNum`,`rt_Type`,`rt_Remark`) VALUES 
				('', '{$userID}','{$userName}','{$userStationName}','{$CurTime}','{$ticketno}','{$ticketno}','{$endticket}','{$ticketnum}','结算单','{$resetreason}')";
			$query = $class_mysql_default->my_query($insert);
			if($queryupdate&&$query){
				$class_mysql_default->my_query("COMMIT");
				echo "<script>alert('结算单重置成功！')</script>";
			}else{
				$class_mysql_default->my_query("ROLLBACK");
				echo "<script>alert('结算单重置失败！')</script>";
			}
		}
		$class_mysql_default->my_query("END TRANSACTION");
		echo "<script>location.assign('tms_v1_checkin_resetingticket.php');</script>";
	}
?>
