...
function addnorunsadjustprice(){
	window.location.href='tms_v1_schedule_addnorunsadjustprice.php?clnumber1='+document.getElementById("NoOfRunsID").value+'&clnumber2='+document.getElementById("LineID").value;	
}
function retur(){
	window.close();
}
...
function modnorunsadjustprice(){
	if (!document.getElementById("ID1").value){
		...
		window.location.href='tms_v1_schedule_modnorunsadjustprice.php?op=mod&clnumber='+document.getElementById("ID1").value;
	}
}
...
function delprice(){
	if (!document.getElementById("ID1").value){
		...
		if(!confirm("确定要删除该班次票价信息吗？")){
			...
			jQuery.get(
				'tms_v1_schedule_delnorunsadjustprice.php',
				...
				function(data){
					...
					if( objData.retVal=='FAIL'){
						...
						alert(objData.retString);
						window.location.href='tms_v1_schedule_modprice.php?clnumber='+document.getElementById("NoOfRunsID").value;
					}
			});
		}
	}
...
<table width="100%" align="center" class="main_tableboder" border="1" cellpadding="3" cellspacing="1">
 <tr>
    ...
    <td colspan="2" bgcolor="#FFFFFF" nowrap="nowrap" >
