<?php
	...
	if ($_GET['op'] == "del" ){
		...
		//	</h2></div>");
			echo "<script>alert('删除成功！ 请返回。');location.assign('tms_v1_basedata_searrunsdock.php?op=see&clnumber=$NoOfRunsID');</script>";
		}else{
			mysql_query("ROLLBACK");
			echo "<script>alert('删除失败！');location.assign('tms_v1_basedata_searrunsdock.php?op=see&clnumber=$NoOfRunsID');</script>";
		}
