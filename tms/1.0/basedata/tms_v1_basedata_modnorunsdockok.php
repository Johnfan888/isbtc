<?php
	...
	$selects="select * from tms_bd_NoRunsDockSite where nds_NoOfRunsID='{$NoOfRunsID}' and nds_SiteName='{$SiteName}'";
	...
	if(!mysql_fetch_array($seles)||$SiteID==$SiteIDD){
		...
		if($query){
			...
			echo"<script>alert('修改失败');window.location.href='tms_v1_basedata_searrunsdock.php?op=see&clnumber=$NoOfRunsID'</script>";
		}
	}else{
		echo"<script>alert('所选站点已存在，请重新选择！');window.location.href='tms_v1_basedata_searrunsdock.php?op=see&clnumber=$NoOfRunsID'</script>";	
	}
