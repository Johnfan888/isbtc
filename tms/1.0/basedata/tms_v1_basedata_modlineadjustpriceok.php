...
//	$NoRunsAdjust=$_POST['NoRunsAdjust'];
	...
	$Remark=$_POST['Remark'];
	$selects="select * from tms_bd_NoRunsAdjustPrice where nrap_ISUnitAdjust='{$ISUnitAdjust}'and nrap_Unit='{$Unit}' and nrap_ISLineAdjust='{$ISLineAdjust}' 
...
//	echo $result;
	if(!$result){
		$update="update tms_bd_NoRunsAdjustPrice set nrap_ISUnitAdjust='{$ISUnitAdjust}',nrap_Unit='{$Unit}',nrap_ISLineAdjust='{$ISLineAdjust}',
			nrap_LineAdjust='{$LineAdjust}',nrap_ISNoRunsAdjust='0',nrap_NoRunsAdjust='NULL',nrap_DepartureSiteID='{$DepartureSiteID}',
	...
	//	if (!$query) echo "SQL错误：".mysql_error();
		if($query){
			...
			echo"<script>alert('修改失败');window.location.href='tms_v1_basedata_searlineadjustprice.php?clnumber=$LineAdjust'</script>";
		}
	}else{
		echo"<script>alert('所填的信息已存在，请重新选择！');window.location.href='tms_v1_basedata_searlineadjustprice.php?clnumber=$LineAdjust'</script>";	
	} */
?>
