<?php
	...
	if(!mysql_fetch_array($seles)){
		...
		if($query1 && $query){
			...
			echo"<script>alert('添加失败');window.location.href='tms_v1_basedata_addnorunsdock.php?NoOfRunsID=$NoOfRunsID&LineID=$LineID'</script>";
		}
		$class_mysql_default->my_query("END TRANSACTION");
	}else{
		echo"<script>alert('所选站点已存在，请重新选择！');window.location.href='tms_v1_basedata_addnorunsdock.php?NoOfRunsID=$NoOfRunsID&LineID=$LineID'</script>";	
	}
?>
