...
function getid(str, obj){
	var st=str.split(',')
	var objid=obj+'ID'
	var obji=obj+'I'
	document.getElementById(obj).value=st[0]
	document.getElementById(objid).value=st[1]
	document.getElementById(obji).value=st[1]
}
...
function sear(){
	window.location.href='tms_v1_basedata_searservicefeeadjust.php?clnumber='+document.getElementById("NoRunsAdjust").value;
}
$(document).ready(function(){
	$("#mod").click(function(){
		...
		jQuery.get(
			'tms_v1_basedata_modservicefeeadjustok.php',
			{'op': 'modservice', 'LineAdjust':$("#LineAdjust").val(), 'ISUnitAdjust':$("#ISUnitAdjust").val(),'ISNoRunsAdjust':$("#ISNoRunsAdjust").val(),
				...
				'EndDate':$("#EndDate").val(),'RunPrice':$("#RunPrice").val(),'Remark':$("#Remark").val(),'ID':$("#ID").val(),'NoRunsAdjust':$("#NoRunsAdjust").val(),
				'time': Math.random()},
			function(data){
			//	alert(data);
				...
				if(objData.retVal=='FAIL'){
					...
					if(objData.retVal=='FAIL1'){
						...
						alert(objData.retString);
						window.location.href='tms_v1_basedata_modservicefeeadjust.php?clnumber='+document.getElementById("ID").value;
					}
				}
		}); 
	});
...
<table width="100%" align="center" class="main_tableboder" border="1" cellpadding="3" cellspacing="1">
  <tr>
    ...
    <span class="graytext" style="margin-left:8px;">修 改 班 次 服 务 费 </span></td>
  </tr>
</table>
<?
//连接数据库，获取班次信息
?>
<div><form name="addL" id="addL" action="tms_v1_basedata_modservicefeeadjustok.php" method="post">
<table width="40%" align="center" class="main_tableboder" border="1" cellpadding="3" cellspacing="1">
	<tr>
		...
		<td bgcolor="#FFFFFF"><input type="text" name="LineAdjust1" id="LineAdjust1" disabled="disabled" style="width:230px;" value="<?php echo $resultsf['sfa_LineAdjust'];?>"/>
		<input type="hidden" name="LineAdjust" id="LineAdjust"  value="<?php echo $resultsf['sfa_LineAdjust'];?>"/>
	...
	<tr> 
    	...
    	<td bgcolor="#FFFFFF"><input type="hidden" name="NoRunsAdjust" id="NoRunsAdjust"  value="<?php echo $resultsf['sfa_NoRunsAdjust'];?>"/>
    	<input type="text" style="width:230px;" name="NoRunsAdjust1" id="NoRunsAdjust1" disabled="disabled" value="<?php echo $resultsf['sfa_NoRunsAdjust'];?>"/>
    	</td>
	</tr> 
	<tr>
		...
		<td bgcolor="#FFFFFF"><input type="radio" name="radio1" id="ISUnit" <?php if($resultsf['sfa_ISUnitAdjust']==1) echo 'checked';?> disabled="disabled">按协议调价
    		<input type="radio" name="radio1" id="ISNoRuns" <?php if($resultsf['sfa_ISNoRunsAdjust']==1) echo 'checked';?> disabled="disabled">按班次调价
    		<input type="hidden" name="ISUnitAdjust" id="ISUnitAdjust" value="<?php echo $resultsf['sfa_ISUnitAdjust'];?>"/>
    		<input type="hidden" name="ISNoRunsAdjust" id="ISNoRunsAdjust" value="<?php echo $resultsf['sfa_ISNoRunsAdjust'];?>"/>
    	</td>
	</tr>
