...
$(document).ready(function(){
	...
	$("#mod").click(function(){
		...
		jQuery.get(
			'tms_v1_basedata_modnorunsadjustpriceok.php',
			{'op': 'modrunprice', 'LineAdjust':$("#LineAdjust").val(), 'ISUnitAdjust':$("#ISUnitAdjust").val(),'ISNoRunsAdjust':$("#ISNoRunsAdjust").val(),
				...
				'BalancePrice':$("#BalancePrice").val(),'Remark':$("#Remark").val(),'ID':$("#ID").val(),'NoRunsAdjust':$("#NoRunsAdjust").val(),'time': Math.random()},
			function(data){
			//	alert(data);
				...
				if(objData.retVal=='FAIL'){
					...
					if(objData.retVal=='FAIL1'){
						...
						alert(objData.retString);
						window.location.href='tms_v1_basedata_modnorunsadjustprice.php?clnumber='+document.getElementById("ID").value;
					}
				}
		}); 
	});
...
<table width="100%" align="center" class="main_tableboder" border="1" cellpadding="3" cellspacing="1">
  <tr>
    ...
    <span class="graytext" style="margin-left:8px;">修 改 班 次 调 价 </span></td>
  </tr>
</table>
<?
//连接数据库，获取班次信息
?>
<div><form name="addL" id="addL" action="tms_v1_basedata_modnorunsadjustpriceok.php" method="post">
<table width="40%" align="center" class="main_tableboder" border="1" cellpadding="3" cellspacing="1">
	<tr>
		...
		<input type="text" style="width:230px;" name="LineAdjust1" id="LineAdjust1" disabled="disabled" value="<?php echo $results['nrap_LineAdjust'];?>"/>
	...
	<tr> 
    	...
    	<td bgcolor="#FFFFFF"><input type="hidden" name="NoRunsAdjust" id="NoRunsAdjust"  value="<?php echo $results['nrap_NoRunsAdjust'];?>"/>
    	<input type="text" name="NoRunsAdjust1" id="NoRunsAdjust1" disabled="disabled" value="<?php echo $results['nrap_NoRunsAdjust'];?>"/></td>
	</tr>
	<tr>
		...
		<td bgcolor="#FFFFFF"><input type="radio" name="radio1" id="ISUnit" <?php if($results['nrap_ISUnitAdjust']==1) echo 'checked';?> disabled="disabled">按协议调价
    		<input type="radio" name="radio1" id="ISNoRuns" <?php if($results['nrap_ISNoRunsAdjust']==1) echo 'checked';?> disabled="disabled">按班次调价
    		<input type="hidden" name="ISUnitAdjust" id="ISUnitAdjust" value="<?php echo $results['nrap_ISUnitAdjust'];?>"/>
    		<input type="hidden" name="ISNoRunsAdjust" id="ISNoRunsAdjust" value="<?php echo $results['nrap_ISNoRunsAdjust'];?>"/>
		</td>
	</tr>
	<tr id="adjustunit" style="display:<?php if($results['nrap_ISUnitAdjust']==1) echo ''; else echo 'none';?>">
...
<!-- 
 ...
 -->
	...
	<tr> 
    	...
    	<td bgcolor="#FFFFFF"><input type="text" name="HalfPrice" id="HalfPrice" value="<?php echo $results['nrap_HalfPrice'];?>" onkeyup="return isnumber(this.value,this.id)"/></td>
	</tr>
	<tr> 
