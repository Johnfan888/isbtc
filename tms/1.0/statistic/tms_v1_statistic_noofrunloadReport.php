...
if(isset($_POST['resultquery']) || isset($_POST['exceldoc'])) {
	...
	if(isset($_POST['exceldoc'])) {
		...
		$out = array($qrydate, '', '', '', '', '');
		fputcsv($fp, $out);
		$head = array('班次', '线路编号', '车站名称', '核定运量', '实际运量', '实载率(%)');
		fputcsv($fp, $head);
		...
		$outputRow = "";
		$ratio = 0;
		$queryString = "SELECT bh_NoOfRunsID AS NoOfRunsID, bh_LineID AS LineID, bh_Station AS Station, IFNULL(SUM(bh_CheckTotal),0) AS CheckTotal, 
						SUM(IFNULL((SELECT tml_TotalSeats FROM tms_bd_TicketMode WHERE (tml_NoOfRunsID = tms_acct_BalanceInHand.bh_NoOfRunsID) 
						AND (tml_NoOfRunsdate = tms_acct_BalanceInHand.bh_NoOfRunsdate) AND (tml_NoOfRunsdate >= '{$CheckBeginDate}') AND (tml_NoOfRunsdate <= '{$CheckEndDate}')),0)) AS SeatS 
						FROM tms_acct_BalanceInHand	WHERE (bh_NoOfRunsdate >= '{$CheckBeginDate}') AND (bh_NoOfRunsdate <= '{$CheckEndDate}') 
						AND (bh_Station LIKE '{$StationName}') GROUP BY bh_NoOfRunsID, bh_Station ORDER BY bh_NoOfRunsID ASC";
		...
		while ($row = mysql_fetch_array($result)) {
			...
			$outputRow = array($row['NoOfRunsID'], $row['LineID'], $row['Station'], $row['SeatS'], $row['CheckTotal'], number_format("$ratio",2)); 
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<table width="100%" align="center" class="main_tableborder" border="1" cellpadding="3" cellspacing="1">
			<tr>
				<td bgcolor="#f0f8ff"><img src="../ui/images/tb.gif" style="top:3px; position:relative;" width="14" height="14" /><span class="graytext" style="margin-left:8px;"> 班 次 运 量 统 计</span></td>
			</tr>
		</table>
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="main_tableborder" style="margin-top:-20px;">
  			<tr>
  				<td align="center" bgcolor="#FFFFFF"><span style="font-weight:bold;font-size:14;color:black;">班次运量汇总表</span></td>
  			</tr>
		</table>
		...
		<thead class="fixedHeader">
			<tr align="center">
				<th nowrap="nowrap" bgcolor="#006699">班次</th>
			...
			?>
			<tr align="center">
				<td nowrap="nowrap"><?php echo $row['NoOfRunsID'];?></td>
